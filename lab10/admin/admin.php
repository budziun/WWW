...
function DodajNowaPodstrone() {
 ...
 function ListaKategorii() {
    ...
    $query = "SELECT id, nazwa, matka FROM categories";
    $result = mysqli_query($conn, $query);
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            $id = $row['id'];
            $nazwa = $row['nazwa'];
            $matka = $row['matka'] == 0 ? 'Brak' : $row['matka'];
...
function DodajKategorie() {
    ...
    $wynik .= 'Matka (ID): <input class="matka" type="number" name="matka" value="0"><br /> <br />';
    $wynik .= '<input class="zapisz" type="submit" value="Dodaj">'.'</form>';
...
function EdytujKategorie() {
    ...
    if (mysqli_num_rows($result) > 0 && $result) {
        ...
        $wynik .= 'Matka (ID): <input class="matka" type="number" name="matka" value="'.$matka.'"><br /> <br />';
        $wynik .= '<input class="zapisz" type="submit" name="zapisz" value="Zapisz">'.'</form>';
